...
const levelLabel: Record<string, string> = {
  Beginner: 'Start here — beginner programs',
  Intermediate: 'Level up — intermediate programs',
  Advanced: 'Go all in — advanced programs',
  'All levels': 'For everyone',
}

const levelBlurb: Record<string, string> = {
  Beginner: 'New to training or returning after a break. We build the foundations safely and effectively.',
  Intermediate: 'Consistent with 6 months – 3 years. Program to push to wall harder and break plateaus.',
  Advanced: 'Trained 3+ years. Advanced periodization for serious strength, size and performance.',
  'All levels': 'Scalable programming that fits wherever you are on your journey.',
}
...
export function ProgramsSection({ limit }: { limit?: number }) {
  const full = limit ? PROGRAMS.slice(0, limit) : PROGRAMS

  // Group programs by level, ordered Beginner → Intermediate → Advanced → All levels.
  const groups = LEVEL_ORDER.map((level) => ({ level, items: full.filter((p) => p.level === level) })).filter((g) => g.items.length > 0)
    ...
    <section id="programs" className="section-padding relative">
      <div className="container-shell">
        <SectionHeading
          ...
          description="Every program is fully personalized after your onboarding assessment — workouts, nutrition and weekly coaching included. Start with your level — one program at a time."
        />
          ...
          {groups.map((group) => (
            <div key={group.level}>
              <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-2xl font-black tracking-tight text-content">{levelLabel[group.level]}</h2>
                  <p className="mt-1 max-w-xl text-sm text-content-muted">{levelBlurb[group.level]}</p>
                </div>
                <Badge className={levelColor[group.level]}>{group.level} · {group.items.length} program{group.items.length > 1 ? 's' : ''}</Badge>
              </div>
        ...
        <div className="mt-12 text-center">
          ...
          >
            Explore all {PROGRAMS.length} programs <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </section>
